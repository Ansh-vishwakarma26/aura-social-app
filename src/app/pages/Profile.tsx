...
import { ArrowLeft, MapPin, Link as LinkIcon, Calendar, Grid, Bookmark, Users, X, User, Image, UserX, MoreHorizontal, ShieldBan, MicOff, Star } from "lucide-react";
...
export const Profile = () => {
  ...
  if (loading) return (
    <div className="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-2xl md:border md:border-zinc-200 dark:border-white/10 md:rounded-3xl overflow-hidden pb-8 animate-pulse">
      <div className="h-32 md:h-48 bg-zinc-200 dark:bg-zinc-700" />
  ...
  return (
    <div className="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-2xl md:border md:border-zinc-200 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.05)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] md:rounded-3xl overflow-hidden pb-8 relative transition-colors duration-300">
      ...
      {/* Network Modal */}
      {networkModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) setNetworkModal(null); }}>
          <div className="bg-white dark:bg-zinc-800 rounded-2xl w-full max-w-sm max-h-[80vh] flex flex-col overflow-hidden border border-zinc-200 dark:border-white/10 shadow-2xl">
            <div className="flex items-center justify-between p-4 border-b border-zinc-200 dark:border-white/10">
              ...
              <button onClick={() => setNetworkModal(null)} className="p-2 -mr-2 text-zinc-400 hover:text-zinc-700 dark:hover:text-white rounded-full hover:bg-zinc-100 dark:hover:bg-white/10">
                <X className="w-5 h-5" />
              </button>
            </div>
      ...
      {/* Mobile Header */}
      <div className="md:hidden sticky top-0 z-10 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-md border-b border-zinc-200 dark:border-white/10 px-4 h-14 flex items-center gap-4 transition-colors duration-300">
        {!isOwnProfile && (
        ...
        <span className="font-bold text-lg text-zinc-900 dark:text-white">{user.username}</span>
      </div>

      {/* Cover & Avatar */}
      <div className="relative">
        <div 
          className="h-32 md:h-48 w-full bg-zinc-100 dark:bg-zinc-800 overflow-hidden"
          style={user.coverImage?.startsWith('#') ? { backgroundColor: user.coverImage } : {}}
        >
          {user.coverImage && !user.coverImage.startsWith('#') && (
            <img src={user.coverImage} className="w-full h-full object-cover" alt="Cover" />
          )}
        </div>
        <div className="absolute -bottom-12 left-4 md:left-8 ring-4 ring-white dark:ring-zinc-900 rounded-full bg-white dark:bg-zinc-900">
          <Avatar src={user.avatar} size="xl" className="w-24 h-24 md:w-32 md:h-32" />
        </div>
      </div>

      {/* Profile Info */}
      <div className="px-4 md:px-8 mt-14 md:mt-16 mb-6">
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex gap-2">
            {isOwnProfile ? (
              ...
              <>
                <Button variant="secondary" onClick={() => navigate(`/messages/${user.username}`)}>
                  Message
                </Button>
                <Button
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="secondary" size="icon" className="w-10 h-10 px-0">
        ...
        <p className="text-[15px] whitespace-pre-line text-zinc-700 dark:text-zinc-300 mb-4 leading-relaxed">{user.bio}</p>

        <div className="flex items-center gap-3 mb-4">
          <span className="text-sm text-zinc-500 dark:text-zinc-400">Personality Type:</span>
          <span className="px-3 py-1 bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-emerald-500/30 rounded-full text-sm font-semibold text-emerald-400">{user.mbti}</span>
        </div>
      ...
      <div className="flex border-t border-zinc-200 dark:border-white/10">
        <button
          ...
          className={cn("flex-1 flex items-center justify-center gap-2 py-3.5 text-sm font-semibold uppercase tracking-wider transition-colors",
      ...
      {/* Grid */}
      <div className={cn("mt-0.5", user.isPrivate && !isOwnProfile && !following ? "flex flex-col items-center justify-center py-20 px-4 text-center border-t border-zinc-200 dark:border-white/10" : "grid grid-cols-3 gap-0.5 sm:gap-1")}>
        {user.isPrivate && !isOwnProfile && !following ? (
          <>
            <div className="w-20 h-20 rounded-full border-2 border-zinc-200 dark:border-zinc-800 flex items-center justify-center mb-6">
          ...
          (activeTab === 'posts' ? posts : savedPosts).map(post => (
            <div key={post.id} className="aspect-square bg-zinc-100 dark:bg-zinc-800 relative group cursor-pointer" onClick={() => navigate(`/post/${post.id}`)}>
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 text-white">
                <div className="flex items-center font-bold gap-1">❤️ {post.likes}</div>
              </div>
            </div>
          ))
        ) : (
          <div className="col-span-3 py-16 flex flex-col items-center justify-center text-zinc-500 dark:text-zinc-400">
            ...
            <p>{activeTab === 'saved' ? 'Posts you save will appear here.' : 'Share your first photo on Aura.'}</p>
          </div>
        )}
      </div>
    </div>
